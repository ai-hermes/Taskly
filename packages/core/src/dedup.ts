import type { TodoItem, Tombstone } from "./types.js";
...
export interface DedupResult<T extends TodoItem = TodoItem> {
 ...
 */
export function dedupTodos<T extends TodoItem>(
  incoming: T[],
  existing: TodoItem[],
  ...
  const liveTombstones =
    ttlMs > 0 ? tombstones.filter((t) => t.deletedAt + ttlMs > now) : [];
 ...
 */
export function dedupPendingTodos<T extends TodoItem>(
  todos: T[],
  threshold: number = SIMILARITY_THRESHOLD
): T[] {
