...
export const EXTRACT_PROMPT = `你是一个待办事项提取助手。请从以下聊天文本中识别出待办事项。
...
export class OpenAIProvider implements LLMProvider {
  ...
  async extractTodos(ocrText: string, options: ExtractTodoOptions = {}): Promise<TodoItem[]> {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `LLM request failed: ${response.status} ${response.statusText}${detail ? ` — ${detail.slice(0, 300)}` : ""}`
      );
    }
      ...
      return details
        .filter((region) => {
          ...
          return normKeywords.some(
            (keyword) => normText.includes(keyword) || keyword.includes(normText)
          );
        })
        .slice(0, 8);
    }
